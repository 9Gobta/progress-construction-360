...
export async function POST(request: Request, context: { params: Promise<{ projectId: string; mediaId: string }> }) {
  ...
  try {
    return NextResponse.json(await apiRequest(`/projects/${projectId}/media/${mediaId}/multipart/parts`, {
      method: "POST",
      token,
    ...
    return NextResponse.json({ detail: error instanceof Error ? error.message : "ขอ Upload URL ไม่สำเร็จ" }, { status });
  }
}
