...
export async function POST(request: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    const capture = await apiRequest(`/projects/${projectId}/captures`, {
      method: "POST",
      token,
    ...
    return NextResponse.json({ detail: error instanceof Error ? error.message : "สร้าง Capture ไม่สำเร็จ" }, { status });
  }
}
