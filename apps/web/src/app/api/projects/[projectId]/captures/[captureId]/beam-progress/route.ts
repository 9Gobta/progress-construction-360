...
export async function PUT(
  ...
  try {
    const result = await apiRequest(`/projects/${projectId}/captures/${captureId}/beam-progress`, {
      method: "PUT", token, body: JSON.stringify(await request.json()),
    });
    return NextResponse.json(result);
  } catch (error) {
    const status = error instanceof ApiError ? error.status : 500;
    return NextResponse.json({ detail: error instanceof Error ? error.message : "บันทึก Progress คานไม่สำเร็จ" }, { status });
  }
}
